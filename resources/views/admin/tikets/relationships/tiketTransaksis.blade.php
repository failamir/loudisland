...
<div class="card">
    ...
    <div class="card-body">
        <div class="table-responsive">
            <table class=" table table-bordered table-striped table-hover datatable datatable-tikettransactions">
                <thead>
                    <tr>
                        <th width="10">
                ...
                <tbody>
                    @foreach($transactions as $key => $transaksi)
                        <tr data-entry-id="{{ $transaksi->id }}">
                            ...
                            <td>
                                @can('transaksi_show')
                                    <a class="btn btn-xs btn-primary" href="{{ route('admin.transactions.show', $transaksi->id) }}">
                                        {{ trans('global.view') }}
                                    </a>
                                @endcan

                                @can('transaksi_edit')
                                    <a class="btn btn-xs btn-info" href="{{ route('admin.transactions.edit', $transaksi->id) }}">
                                        {{ trans('global.edit') }}
                                    </a>
                                @endcan

                                @can('transaksi_delete')
                                    <form action="{{ route('admin.transactions.destroy', $transaksi->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@endcan
  ...
  let table = $('.datatable-tikettransactions:not(.ajaxTable)').DataTable({ buttons: dtButtons })
