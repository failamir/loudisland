...
export default function RolesListPage() {
  ...
  const onAdd = () => { setEditingItem(null); setModalOpen(true); };
  const onEdit = (item) => { setEditingItem(item); setModalOpen(true); };
  const onDelete = async (item) => {
    if (!confirm(`Hapus role ${item.title || item.name}?`)) return;
    try {
      await axios.delete(`${baseUrl}/roles/${item.id}`);
  ...
  const columns = useMemo(
    () => [
      ...
      },
      {
        ...
        id: 'title',
        header: ({ column }) => (
          <DataGridColumnHeader title="Role" filter={<ColumnInputFilter column={column} />} column={column} />
        ),
        enableSorting: true,
        cell: ({ row }) => (
      ...
      },
      {
        ...
        enableSorting: false,
        cell: ({ row }) => (row.original.permissions || []).map(p => p.title || p.name).join(', '),
            ...
            <ToolbarHeading>
              ...
              <ToolbarDescription>
                <div className="flex items-center flex-wrap gap-1.5 font-medium">
                  <span className="text-md text-gray-700">Total Roles:</span>
